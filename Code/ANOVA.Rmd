...
```{r}
anova.1 <- AllData %>% filter(Day == "D1") %>% aov(data = ., EggCount ~ Condition)
summary(anova.1)
TukeyHSD(anova.1)
```

```{r}
anova.3 <- AllData %>% filter(Day == "D3") %>% aov(data = ., EggCount ~ Condition)
summary(anova.3)
TukeyHSD(anova.3)
```

```{r}
anova.7 <- AllData %>% filter(Day == "D7") %>% aov(data = ., EggCount ~ Condition)
summary(anova.7)
TukeyHSD(anova.7)
```
